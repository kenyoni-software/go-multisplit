...
func registerFlags(fSet *flag.FlagSet, cfg *Settings) {
	...
	const blockVarDesc = "when splitting, use a block instead of separate 'var' lines"
	fSet.BoolVar(&cfg.Assign, "assign", cfg.Assign, "split multiple assignments (e.g. 'a, b = 1, 2')")
	fSet.BoolVar(&cfg.ConstDeclFunc, "const-decl-func", cfg.ConstDeclFunc, "split multiple const declarations in function scope")
	...
	fSet.BoolVar(&cfg.VarDeclInitFunc, "var-decl-init-func", cfg.VarDeclInitFunc, "split multiple var declarations with initializer in function scope")
